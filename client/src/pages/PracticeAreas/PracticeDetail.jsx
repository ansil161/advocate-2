...
export default function PracticeDetail() {
  ...
  return (
    <Layout navTheme="dark">
      {/* Title and description come straight off the practice entry, so a new
          domain added to data/practiceAreas.js is indexable without touching
          this file. `short` is already written as a one-line summary. */}
      <PageMeta
        title={p.title}
        description={p.short}
      ...
      <section className="pd-block pd-block--alt" id="pd-counsel">
        <div className="container pd-counsel">
          <span className="chapter-label"><Icon name="person" /> Lead Counsel</span>
          <Reveal className="pd-counsel__card" amount={0.15}>
            <CounselCard advocate={lead}>
              <Link to={`/team#${lead.slug}`} className="link-arrow link-arrow--light">
      ...
      <section className="pd-block" id="pd-related">
        <div className="container">
          <span className="chapter-label"><Icon name="grid" /> Related Areas</span>
          <div className="pd-related__grid">
            {related.map((r, i) => (
              <Reveal as={Link} to={`/practice/${r.slug}`} className="pa-card" key={r.slug} delay={i * 0.08}>
                <span className="pa-card__num"><Icon name={practiceIcon(r.slug)} /></span>
      ...
      {p.faq.length > 0 && (
        <section className="pd-block pd-block--alt" id="pd-faq">
          <div className="container">
            <span className="chapter-label"><Icon name="question" /> Questions</span>
